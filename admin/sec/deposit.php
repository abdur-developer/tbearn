<div class="report-container">
    <div class="report-header">
        <h1 class="recent-Articles">Deposit</h1>
        <!--  -->
    </div>
    ...
    <table class="report-body package">
        ...
        <tbody class="items">
            <?php 
            $user_query = mysqli_query($conn, "SELECT * FROM deposit");
            if(mysqli_num_rows($user_query) > 0){
                while($row = mysqli_fetch_assoc($user_query)){ ?>
                <tr>
                    <td><h3 class="t-op-nextlvl"><?= $row['trx_id'] ?></h3></td>
                    ...
                    <?php
                        ...
                        if($row['status'] == 0){
                            ...
                            echo "<td><h3 class='t-op-nextlvl'>
                                    <a href='?q=deposit&dip_action=app&id=$dip_id' class='view-true'>Approve</a>
                                    <a href='?q=deposit&dip_action=rej&id=$dip_id' class='view-false'>Reject</a>
                                </h3></td>";
                        }elseif($row['status'] == 2){
                            echo "<td><h3 class='t-op-nextlvl label-tag-red'>Banned</h3></td>";
                            echo "<td><h3 class='t-op-nextlvl'>
                                    <a href='?q=deposit&dip_action=del&id=$dip_id' class='view-false'>Delete</a>
                                </h3></td>";
                        }else{
